  ...
  export const basketReducer = (state = defaultState, action) => {
	switch (action.type) {
	  case ADD_PRODUCT_TO_BASKET: {
		...
		const priceToAdd = parseFloat(productToAdd.discont_price || productToAdd.price) * productToAdd.count;
	  ...
	  case INCR_PRODUCT_COUNT: {
		...
		const updatedTotalPrice =
		  state.basket.totalPrice + parseFloat(product.discont_price || product.price);
	  ...
	  case DECR_PRODUCT_COUNT: {
		...
		const updatedTotalPrice =
		  state.basket.totalPrice -
		  (product.count > 1 ? parseFloat(product.discont_price || product.price) : 0);
